...
resource "azurerm_subscription_policy_assignment" "subscriptions" {
  ...
  policy_definition_id = lookup(var.initiative_name_id_map_subscription, each.value.display_name)
...
resource "azurerm_management_group_policy_assignment" "management_groups" {
  ...
  name                 = each.value.name
  policy_definition_id = lookup(var.initiative_name_id_map_management_group, each.value.display_name)
